...
type PlantGameState = 'active' | 'wilted' | 'sleeping' | 'completed';
...
const renderFormattedDescription = (description: string) => {
	return description.split('\n').map((line, index) => {
		...
		if (!trimmed) return <div key={`spacer-${index}`} style={{ height: 6 }} />;
		if (trimmed.endsWith(':')) {
			return (
				<Text key={`heading-${index}`} c='teal.2' fw={700} mt={4}
		...
		if (trimmed.startsWith('- ')) {
			return (
				<Text key={`bullet-${index}`} c='gray.2'
		...
		return (
			<Text key={`paragraph-${index}`} c='gray.2'
...
const EMPTY_SLOTS: PlantSlot[] = Array.from({ length: 8 }, (_, i) => ({ slot_index: i, plant: null }));

export const BioGardenGame = () => {
	const navigate = useNavigate();
	const [slots, setSlots] = useState<PlantSlot[]>(EMPTY_SLOTS);
	...
	const updatePlantInState = useCallback((plantId: number, updates: Partial<BioGardenPlant>) => {
		setSlots(prev => prev.map(s =>
			s.plant?.id === plantId ? { ...s, plant: { ...s.plant!, ...updates } } : s,
		));
	}, []);
	...
	const fetchPlants = useCallback(async () => {
		try {
			...
			const normalizedSlots = data.slots.map(s => ({
				...s,
				plant: s.plant ? { ...s.plant, current_stage: Math.max(s.plant.current_stage || 0, 0) } : null,
			}));
	...
	// Сброс панели при смене выбранного слота
	useEffect(() => {
		setPanelMode('info');
	...
	useEffect(() => { stopSpeaking(); }, [selectedSlotIndex, stopSpeaking]);
	...
	const speakText = useCallback((text: string) => {
		...
		const ruVoice = selectedVoice === '__auto__'
	...
	const handleSlotClick = (slotIndex: number) => {
		...
		if (!slot) return;
		if (!slot.plant) {
			// Пустой горшок → открыть выбор растения
	...
	const handleDeselect = () => { setSelectedSlotIndex(null); };

	const handleRandomPlant = () => {
		const occupied = slots.filter(s => s.plant);
		if (occupied.length === 0) return;
		const random = occupied[Math.floor(Math.random() * occupied.length)];
		if (random) setSelectedSlotIndex(random.slot_index);
	};

	const handlePickPlant = async (plant: AvailablePlant) => {
		if (pendingSlotIndex === null) return;
		setActionLoading(true);
		setActionError(null);
		try {
			const result = await apiService.startPlant(validTelegramId, plant.id, pendingSlotIndex);
			const newPlant: BioGardenPlant = {
				id: plant.id,
				name: plant.name,
				...
				biology_topics: plant.biology_topics,
				is_unlocked: true,
			...
			setSlots(prev => prev.map(s =>
				s.slot_index === pendingSlotIndex ? { ...s, plant: newPlant } : s,
			));
			setAvailablePlants(prev => prev.filter(p => p.id !== plant.id));
			setShowPlantPicker(false);
			setSelectedSlotIndex(pendingSlotIndex);
			setPendingSlotIndex(null);
		} catch (e: unknown) {
			const msg = (e as { response?: { data?: { message?: string } } })?.response?.data?.message;
			setActionError(msg || 'Ошибка при посадке');
		} finally {
			...
			if (isReviveMode) {
				...
				if (result.revived) {
					updatePlantInState(activePlant.id, {
						is_wilted: false, is_unlocked: true,
						health_points: result.health_points ?? 40, revive_sleep_until: null,
					});
				} else if (result.revive_sleep_until) {
					updatePlantInState(activePlant.id, { revive_sleep_until: result.revive_sleep_until });
				}
			} else {
	...
	const renderGameStatus = () => {
		...
		const hpPct = activePlant.max_health_points > 0
			? Math.round((activePlant.health_points / activePlant.max_health_points) * 100) : 0;
		const stagePct = activePlant.growth_stages > 0
			? Math.round((activePlant.current_stage / activePlant.growth_stages) * 100) : 0;
		const hpColor = getHpColor(activePlant.health_points, activePlant.max_health_points || 100);

		return (
			<Paper p='sm' style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 10 }}>
				<Stack gap={8}>
					...
					{gameState === 'active' && (
						<Group gap={8}>
							<Button size='xs' color='blue' style={{ flex: 1 }}
								onClick={() => handleGetQuestion(false)} loading={actionLoading}>
								❓ Ответить на вопрос
							</Button>
							<Button size='xs' color='cyan' variant='light' onClick={handleWaterPlant} loading={actionLoading}>
								💧 Полить (10 🪙)
							</Button>
						</Group>
					)}
					...
					{gameState === 'wilted' && (
						<Stack gap={6}>
							...
							<Group gap={8}>
								<Button size='xs' color='teal' style={{ flex: 1 }}
									onClick={() => handleGetQuestion(true)} loading={actionLoading}>
									🔄 Реанимировать (вопрос)
								</Button>
								<Button size='xs' color='orange' variant='light'
									onClick={handleReviveByCoin} loading={actionLoading}>
									💰 50 🪙
								</Button>
							</Group>
						</Stack>
	...
	const renderQuestionPanel = () => {
		...
		return (
			<Stack gap={12}>
				<Group gap={8} align='center'>
					<Button size='xs' variant='subtle' color='gray' leftSection={<ChevronLeft size={14} />}
						onClick={() => setPanelMode('info')}>
						Назад
					</Button>
					{isReviveMode && <Badge color='orange' size='sm'>Реанимация</Badge>}
					<Badge color='blue' size='xs' variant='light'>{currentQuestion.biology_topic}</Badge>
					{currentQuestion.ege_code && (
						<Badge color='gray' size='xs' variant='outline'>ЕГЭ {currentQuestion.ege_code}</Badge>
					)}
				</Group>
				<Text c='white' fw={600} style={{ fontSize: 'clamp(14px, 2vw, 18px)', lineHeight: 1.45 }}>
					{currentQuestion.question_text}
				</Text>
				<Stack gap={8}>
					{currentQuestion.options.map((opt, idx) => (
						<Button key={opt.id} variant='light' color='blue' justify='flex-start' fullWidth
							disabled={actionLoading} onClick={() => handleSubmitAnswer(opt.id)}
							style={{ textAlign: 'left', height: 'auto', minHeight: 44, whiteSpace: 'normal', padding: '8px 14px' }}>
	...
	const renderResultPanel = () => {
			...
			return (
				<Stack gap={12}>
					<Group gap={10} align='center'>
						{isCorrect ? <CheckCircle size={28} color='#4ade80' /> : <XCircle size={28} color='#f87171' />}
					...
					{revResult.explanation && (
						<Paper p='sm' style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 8 }}>
							<Text c='gray.3' size='sm' style={{ lineHeight: 1.5 }}>💡 {revResult.explanation}</Text>
						</Paper>
					)}
					<Button color={isCorrect ? 'teal' : 'gray'} fullWidth onClick={handleContinue}>Продолжить</Button>
				</Stack>
			);
		}
		...
		const hpPct = p.max_health_points > 0 ? Math.round((p.health_points / p.max_health_points) * 100) : 0;
			...
			<Stack gap={10}>
				<Group gap={10} align='center'>
					...
					{isCorrect && normResult.combo_count >= 3 && (
						<Badge color='yellow' leftSection={<Zap size={12} />}>Серия {normResult.combo_count}</Badge>
					)}
				</Group>
				{isCorrect && (normResult.earned_experience > 0 || normResult.earned_coins > 0) && (
					<Group gap={12}>
						...
						{normResult.animation === 'stage_up' && <Badge color='teal'>🌱 Новая стадия!</Badge>}
					</Group>
				)}
				...
				{normResult.explanation && (
					<Paper p='sm' style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 8 }}>
						<Text c='gray.3' size='sm' style={{ lineHeight: 1.5 }}>💡 {normResult.explanation}</Text>
					</Paper>
				)}
				<Group gap={8} align='center'>
					<Heart size={14} color='#f87171' style={{ flexShrink: 0 }} />
				...
				{p.is_completed && <Badge color='teal' variant='filled'>✅ Растение полностью выращено!</Badge>}
				<Button color={isCorrect ? 'teal' : 'gray'} fullWidth onClick={handleContinue}>Продолжить</Button>
			</Stack>
		);
	};
	...
	const occupiedCount = slots.filter(s => s.plant).length;
	...
	return (
		<div style={{ position: 'relative', width: '100%', height: '100dvh', overflow: 'hidden' }}>
			...
			{/* Случайное растение */}
			<div style={{ position: 'absolute', top: 'clamp(86px, 13vh, 112px)', left: 12, zIndex: 10, pointerEvents: 'all' }}>
				<Button size='xs' color='green' variant='filled' onClick={handleRandomPlant} disabled={occupiedCount === 0}>
					Случайное растение
				</Button>
			</div>
			...
			<div style={{
				...
				<Group justify='space-between' align='center'>
					<Button leftSection={<ArrowLeft size={15} />} onClick={() => navigate(-1)}
						variant='filled' color='dark' size='xs' style={{ opacity: 0.9, pointerEvents: 'all' }}>
						Назад
					</Button>
					...
					<Group gap={6}>
						<Paper p='5px 10px' style={{ background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(8px)', border: '1px solid rgba(255,255,255,0.1)', pointerEvents: 'all' }}>
							<Group gap={5}>
						...
						<Paper p='5px 10px' style={{ background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(8px)', border: '1px solid rgba(255,255,255,0.1)', pointerEvents: 'all' }}>
							<Group gap={5}>
								...
								<Text c='white' size='xs' fw={600}>{occupiedCount}/8</Text>
							</Group>
						</Paper>
					</Group>
				</Group>
			...
			<AnimatePresence>
				{selectedSlotIndex === null && !showPlantPicker && (
					<motion.div
						initial={{ opacity: 0, y: 10 }}
						animate={{ opacity: 1, y: 0 }}
						exit={{ opacity: 0, y: 10 }}
						transition={{ duration: 0.3 }}
						style={{ position: 'absolute', bottom: 24, left: '50%', transform: 'translateX(-50%)', zIndex: 10, pointerEvents: 'none' }}
					>
						<Paper p='sm' style={{ background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(8px)', border: '1px solid rgba(255,255,255,0.12)' }}>
							<Text c='white' size='sm' ta='center'>
								{occupiedCount === 0
									? 'Нажмите на горшок, чтобы выбрать и посадить растение'
									: 'Нажмите на горшок, чтобы открыть описание и играть'}
							</Text>
						</Paper>
					</motion.div>
				)}
			...
			{/* Нижняя панель (выбранное растение) */}
			<AnimatePresence>
				{activePlant && (
					<motion.div
						initial={{ y: '100%' }}
						...
						style={{ position: 'absolute', bottom: 0, left: 0, right: 0, zIndex: 10, pointerEvents: 'all' }}
					>
						<div style={{
							...
							{panelMode === 'info' && (
								<Stack gap={10}>
									<Group justify='space-between' align='flex-start'>
										...
										<Badge color={getDifficultyColor(activePlant.difficulty_level)} variant='filled' size='sm'>
									...
									<Group gap={8}>
										<Select size='xs' w={220} data={voiceOptions} value={selectedVoice}
											onChange={value => setSelectedVoice(value || '__auto__')} disabled={isSpeaking} />
										<Select size='xs' w={110}
											data={[
									...
									<Group gap={8}>
										<Button size='xs' color='teal' onClick={() => speakText(activePlantDescription)} disabled={isSpeaking}>
											🔊 Озвучить
										</Button>
										<Button size='xs' variant='light' color='blue' onClick={pauseSpeaking} disabled={!isSpeaking || isPaused}>
											⏸ Пауза
										</Button>
										<Button size='xs' variant='light' color='indigo' onClick={resumeSpeaking} disabled={!isSpeaking || !isPaused}>
											▶ Продолжить
										</Button>
										<Button size='xs' variant='light' color='gray' onClick={stopSpeaking} disabled={!isSpeaking}>
											⏹ Стоп
										</Button>
									</Group>
									...
									{activePlant.biology_topics?.length > 0 && (
										<Group gap={4} wrap='wrap'>
											{activePlant.biology_topics.map((topic, i) => (
												<Badge key={i} size='xs' variant='dot' color='teal'>{topic}</Badge>
											))}
										</Group>
									)}
								</Stack>
				...
				{showPlantPicker && (
					...
					>
						<Group justify='space-between' align='center' mb='md'>
							<Text c='white' fw={700} size='xl'>🌱 Выберите растение</Text>
							<Button
								size='xs' variant='subtle' color='gray'
								leftSection={<X size={14} />}
								onClick={() => { setShowPlantPicker(false); setPendingSlotIndex(null); }}
							>
								Закрыть
							</Button>
						</Group>
						...
						{actionError && (
							<Text c='red' size='sm' mb='sm'>{actionError}</Text>
						)}

						{availablePlants.length === 0 ? (
							<Center py='xl'>
								<Text c='dimmed' ta='center'>
									Все растения уже посажены в горшки
								</Text>
							</Center>
						) : (
										...
										<Stack gap={8}>
											<Group justify='space-between' align='flex-start'>
												<div style={{ flex: 1, minWidth: 0 }}>
													<Text c='white' fw={700} style={{ fontSize: 15 }}>{plant.name}</Text>
													<Text c='dimmed' fs='italic' size='xs'>{plant.scientific_name}</Text>
												</div>
												<Badge color={getDifficultyColor(plant.difficulty_level)} size='sm'>
													Ур. {plant.difficulty_level}
												</Badge>
											</Group>

											{plant.description && (
												<Text c='gray.4' size='xs' lineClamp={2} style={{ lineHeight: 1.45 }}>
											...
											<Group gap={6} wrap='wrap'>
												<Badge size='xs' variant='outline' color='teal'>
													{plant.growth_stages} стадий
												</Badge>
												{plant.biology_topics?.slice(0, 2).map((t, i) => (
											...
											>
												🌱 Посадить в горшок {pendingSlotIndex !== null ? pendingSlotIndex + 1 : ''}
											</Button>
										</Stack>
									</Card>
								))}
