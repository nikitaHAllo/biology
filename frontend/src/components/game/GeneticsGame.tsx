...
import type { GeneticScenario, GeneticStep, GeneticOption } from '../../models/genetics';
...
const DIFF_COLOR: Record<string, string> = { easy: '#27ae60', medium: '#f39c12', hard: '#e74c3c' };
...
export const GeneticsGame: React.FC = () => {
	...
	const [resultData, setResultData] = useState<{ score: number; coins: number; title: string } | null>(null);
	...
	function advance() {
		...
		setPlay(p => p ? { ...p, currentIndex: next, feedback: null, pendingAdvance: false } : p);
	}
		...
		if (option.is_correct) {
			setPlay(p => {
				...
				return {
					...
					answeredIds,
					feedback: { optionId: option.id, text: step.explanation ?? 'Верно!', correct: true },
					pendingAdvance: true,
				};
			});
		} else {
			setPlay(p => p ? {
				...p,
				feedback: { optionId: option.id, text: option.feedback ?? 'Неверно, попробуй ещё раз.', correct: false },
			} : p);
		}
	}
	...
	// ── Renders ───────────────────────────────────────────────────────────────
	if (screen === 'list') {
		return <ScenarioList scenarios={scenarios} loading={loading} onSelect={startScenario} />;
	}
	if (screen === 'result' && resultData) {
		return <ResultScreen title={resultData.title} score={resultData.score} coins={resultData.coins} onBack={backToList} />;
	}
	if (screen === 'play' && play) {
		return (
			<PlayScreen
...
const ScenarioList: React.FC<{
	...
	<div style={{ padding: '20px 16px', maxWidth: 600, margin: '0 auto' }}>
		<h1 style={{ margin: '0 0 6px', fontSize: 24 }}>🧬 Генетический калькулятор</h1>
		<p style={{ margin: '0 0 24px', color: '#666', fontSize: 14 }}>Решайте генетические задачи пошагово</p>
		...
		{scenarios.map(s => (
			...
			>
				<div style={{ fontSize: 36, flexShrink: 0 }}>{s.is_completed ? '✅' : '🧬'}</div>
				<div style={{ flex: 1, minWidth: 0 }}>
					<div style={{ fontWeight: 700, fontSize: 16, marginBottom: 4 }}>{s.title}</div>
					{s.description && <div style={{ fontSize: 13, color: '#666', marginBottom: 8 }}>{s.description}</div>}
					<div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
						<span style={{ fontSize: 12, background: DIFF_COLOR[s.difficulty] + '22', color: DIFF_COLOR[s.difficulty], padding: '2px 8px', borderRadius: 20, fontWeight: 600 }}>
							{DIFF_LABEL[s.difficulty]}
						</span>
						<span style={{ fontSize: 12, background: '#fff3cd', color: '#856404', padding: '2px 8px', borderRadius: 20 }}>
							🪙 {s.coins_reward} монет
						</span>
						{s.is_completed && s.score != null && (
							<span style={{ fontSize: 12, background: '#d4edda', color: '#155724', padding: '2px 8px', borderRadius: 20 }}>
								✓ {s.score} очков
							</span>
						)}
					</div>
...
const PlayScreen: React.FC<{
	...
	return (
		<div style={{ maxWidth: 640, margin: '0 auto', padding: '0 0 40px' }}>
			{/* Header */}
			<div style={{ position: 'sticky', top: 0, background: 'var(--mantine-color-body, #fff)', zIndex: 10, padding: '12px 16px', borderBottom: '1px solid #f0f0f0', display: 'flex', alignItems: 'center', gap: 12 }}>
				<button
					onClick={onBack}
					style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 20, color: '#666', padding: '4px 6px', borderRadius: 8 }}
				>
					←
				</button>
				<div style={{ flex: 1, minWidth: 0 }}>
				...
				<div style={{ fontWeight: 700, fontSize: 16, color: '#2980b9', whiteSpace: 'nowrap' }}>
					⭐ {play.score}
				</div>
			</div>
			...
			<div style={{ height: 4, background: '#f0f0f0' }}>
				<div style={{ height: '100%', background: '#2980b9', width: `${((play.currentIndex + 1) / play.steps.length) * 100}%`, transition: 'width 0.3s' }} />
			</div>
...
// ── Step card ─────────────────────────────────────────────────────────────────
const STEP_ICON: Record<string, string> = { info: '📖', question: '❓', result: '🏁' };
const STEP_COLOR: Record<string, string> = { info: '#2980b9', question: '#8e44ad', result: '#27ae60' };
...
const StepCard: React.FC<{
...
}> = ({ step, isActive, isDone, answered, feedback, pendingAdvance, isLast, onAdvance, onSelectOption }) => {
	...
	return (
		...
		>
			{/* Card header */}
			<div style={{ background: isActive ? STEP_COLOR[step.step_type] + '15' : '#fafafa', padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 10, borderBottom: '1px solid #f0f0f0' }}>
				<span style={{ fontSize: 20 }}>{STEP_ICON[step.step_type]}</span>
				<span style={{ fontWeight: 700, fontSize: 15, flex: 1 }}>{step.title}</span>
				{step.step_type === 'question' && step.points > 0 && (
					<span style={{ fontSize: 12, background: '#fff3cd', color: '#856404', padding: '2px 8px', borderRadius: 20 }}>
						+{step.points} очк.
					</span>
				)}
				{isDone && <span style={{ fontSize: 18 }}>{answered ? '✅' : '📌'}</span>}
			</div>

			{/* Card body */}
			<div style={{ padding: '14px 16px' }}>
				<p style={{ margin: '0 0 14px', fontSize: 15, lineHeight: 1.6, whiteSpace: 'pre-wrap' }}>{step.content}</p>
							...
							return (
								<button
									...
									style={{
										...
										background: isCorrect ? '#d4edda' : isWrong ? '#fde8e8' : '#fff',
										cursor: (answered || pendingAdvance) ? 'default' : 'pointer',
										textAlign: 'left',
										fontSize: 14,
										fontWeight: isCorrect ? 700 : 400,
										transition: 'background 0.2s, border-color 0.2s',
										color: isCorrect ? '#155724' : isWrong ? '#721c24' : 'inherit',
									}}
								>
									{isCorrect && '✓ '}{isWrong && '✗ '}{opt.option_text}
								</button>
							);
						})}
				...
				{step.step_type === 'question' && isDone && answered && step.explanation && (
					<div style={{ padding: '8px 12px', borderRadius: 10, background: '#f0f7ff', color: '#2980b9', fontSize: 13, fontStyle: 'italic' }}>
						{step.explanation}
					</div>
				)}

				{/* Advance button */}
				{isActive && (step.step_type === 'info' || step.step_type === 'result' || pendingAdvance) && (
					<button
						onClick={onAdvance}
						style={{
							marginTop: 14,
...
const ResultScreen: React.FC<{
	...
	<div style={{ maxWidth: 480, margin: '0 auto', padding: '48px 16px', textAlign: 'center' }}>
		...
		<div style={{ display: 'flex', justifyContent: 'center', gap: 24, marginBottom: 40 }}>
			<div style={{ background: '#fff', border: '2px solid #e8e8e8', borderRadius: 16, padding: '20px 28px' }}>
				<div style={{ fontSize: 28, fontWeight: 800, color: '#2980b9' }}>{score}</div>
				<div style={{ fontSize: 13, color: '#888', marginTop: 4 }}>очков</div>
			</div>
			{coins > 0 && (
				<div style={{ background: '#fff', border: '2px solid #ffd700', borderRadius: 16, padding: '20px 28px' }}>
					<div style={{ fontSize: 28, fontWeight: 800, color: '#f39c12' }}>+{coins}</div>
		...
		<button
			onClick={onBack}
			style={{ padding: '12px 32px', background: '#2980b9', color: '#fff', border: 'none', borderRadius: 12, cursor: 'pointer', fontSize: 16, fontWeight: 700 }}
		>
			К сценариям
		</button>
	</div>
