...
class ApiService {
  ...
  async getAuthMe(): Promise<AuthUser> {
    const response =
      await this.api.get<ApiResponse<{ user: AuthUser }>>("/auth/me");
  ...
  async getQuizCategories(): Promise<QuizCategoriesResponse> {
    const response = await this.api.get<ApiResponse<QuizCategoriesResponse>>(
      "/quizzes/categories",
    );
    return response.data.data;
  }
  ...
  async getPlantsList(
    ...
    const params =
      telegramId && !isNaN(Number(telegramId)) ? { telegramId } : {};
  ...
  async startPlant(
    ...
    const response = await this.api.post(
      `/biogarden/plants/${plantId}/start`,
      body,
    );
    return response.data.data;
  }
  ...
  async revivePlant(
    ...
    const response = await this.api.post(
      `/biogarden/plants/${plantId}/revive`,
      {
        telegramId,
        mode,
      },
  ...
  async getBiogardenReviveQuestion(
    plantId: number,
    telegramId?: number,
  ): Promise<ReviveQuestionResponse> {
    const params = telegramId && !isNaN(telegramId) ? { telegramId } : {};
    const response = await this.api.get(
      `/biogarden/plants/${plantId}/revive-question`,
      { params },
    );
    return response.data.data;
  }
  ...
  async submitReviveAnswer(
    ...
    if (telegramId && !isNaN(telegramId)) body.telegramId = telegramId;
    const response = await this.api.post(
      `/biogarden/plants/${plantId}/revive-answer`,
      body,
    );
    return response.data.data;
  }
  ...
  async submitVirusAnswer(
    caseId: number,
    chapterId: number,
    optionId: number,
  ): Promise<VirusSubmitAnswerResponse> {
    const response = await this.api.post(
      `/virus/cases/${caseId}/chapters/${chapterId}/answer`,
      { optionId },
    );
    return response.data.data;
  }

  async completeVirusCase(
    caseId: number,
    correctAnswers: number,
    totalChapters: number,
    telegramId?: number,
  ): Promise<CompleteVirusCaseResponse> {
